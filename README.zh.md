...
我个人的 AI / 开发工作区——可复现的配置与笔记,纳入版本管理,方便换机复原、分享给同事。

目前收录我的**终端环境**:**Ghostty + tmux**,针对配合 coding agent(Claude Code 等)做了调校。

➡️ 完整配置、说明与一键复原:**[`terminal/`](./terminal/)**
...
- **快且轻** —— GPU 加速渲染、占用内存低,同时开多个 agent 会话也从容。
- **原生分屏与标签** —— 简单场景无需多路复用器即可分屏/开标签。
- **Shell 集成** —— 追踪当前目录、在长 agent 日志里跳转命令提示符、agent 跑完/等输入时响铃提醒。
- **纯文本配置** —— 单个可读文件,易于版本化与分享。

## 为什么在 Ghostty 之上再用 tmux

终端窗口只是一个**取景器**;tmux 让**会话(session)**独立于窗口存活。这正是 agent 工作流所需:
...
- **跨重启恢复** —— 配 `tmux-resurrect` + `tmux-continuum`,布局与各 pane 的 cwd 自动回来——这是"退出后目录全丢"的彻底解法。

> 一句话:**Ghostty** 是快而顺手的画布;**tmux** 是让会话与 agent 持续存活的运行时。
...
深度阅读/导航,IDE(Cursor)仍更强——保留它。想在 *agent 跑着时不打断心流地快速查一眼*,加这套:
...
```
ccy-ai-workspace/
├── README.md / README.zh.md   ← 本总览(英文 / 中文)
