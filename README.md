...
My personal AI / dev workspace — reproducible configs and notes, version-controlled
so I can restore on a new machine and share with teammates.

Currently it holds my **terminal environment**: **Ghostty + tmux**, tuned for
working with coding agents (Claude Code & friends).

➡️ Full setup, configs and one-command restore: **[`terminal/`](./terminal/)**
...
- **Fast & light** — GPU-accelerated rendering, low memory; comfortable running many
...
- **Plain-text config** — a single readable file, easy to version and share.

## Why tmux (on top of Ghostty)

The terminal window is just a *viewport*; tmux makes the **session** survive
independently of it. That's exactly what agent workflows need:

- **Persistence** — sessions live in a background process. Close the laptop, drop SSH,
  quit Ghostty → long agent jobs keep running; reattach and you're back, *same live
  processes, same working directories*.
...
- **Restore across reboots** — with `tmux-resurrect` + `tmux-continuum`, layout and
  each pane's cwd come back automatically — the thorough fix for "lost my directories
  after quitting".

> In short: **Ghostty** is a fast, pleasant canvas; **tmux** is the durable runtime
> that keeps your sessions and agents alive.
...
plugins once, your layout and each pane's directory restore on the next tmux start.
...
> (`prefix + number`) is instant, so **prefer more windows over more panes**, and use
...
For deep reading/navigation, an IDE (Cursor) is still better — keep it. For a quick
look *without breaking flow while an agent runs*, add: `eza --tree` (structure),
`bat` (view files), `ripgrep` + `fzf` (search/jump), `lazygit` (git), `yazi` (file
manager). Install: `brew install eza bat ripgrep fzf yazi lazygit`. Details in
...
```
ccy-ai-workspace/
├── README.md / README.zh.md   ← this overview (EN / 中文)
...
See [`terminal/`](./terminal/) for the details and the decision log behind each choice.
